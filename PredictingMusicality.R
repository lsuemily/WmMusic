...
#will need to change your path based on your own folder structure
...
#correlation of WMC with Raven's
corr.test (gmsiwmcgf_numeric[c(11, 13, 18,23)], use = "pairwise")


##regression of working memory span tasks predicting General Score
WMCspan = lm(formula = GENERAL ~ OspanPartialScore + SspanPartiaScore + TspanPartScore, data = gmsiwmcgf_numeric)
WMCspan
summary(WMCspan)

RavenPred = lm(formula = GENERAL ~ RavenTotal, data = gmsiwmcgf_numeric)
RavenPred
summary(RavenPred)
